 ...
 */
package fa.gs.utils.restdoc.test;
...
public class Main {
    ...
    private static class Punto extends TObject {
        ...
        public Punto() {
            ...
            property("x", x);
            property("y", y);
            property("z", TArray.of(Types.Float()));
        }
    ...
    public static void test_http(String... args) throws Exception {
        ...
        Header x = Header.instance("X-Device")
                .containedType(Types.Literal(Types.Boolean()).value("true"))
                .description("El cosito.", "Donde se usa.");
        ...
        ex.description(Text.instance("olo", "vorgo"));
        ex.value(Text.instance("olo", "vorgo"));
        ...
        req.path().queryParam(Param.instance("pageSize").description("ajshdakjsdh").example(Example.instance().value(Text.instance("5"))));
        req.header(h);
        req.header(x);
        ...
        File file = new File(".\\viewer\\out.txt");
        file.createNewFile();
